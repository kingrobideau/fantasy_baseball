...
class fangraphs_scraper():
    ...
    def __init__(self, league, player_type, year, table):
        ...
        self.filename = player_type + "_" + table + "_" + year + ".csv"
         ...
         for row in rows:
             ...
             if year2== self.year: #INPUT CHECK: year
                ...
                if (pos == "P" and self.player_type =="pitcher") or (pos != "P" and self.player_type == "batter"): #INPUT CHECK: player_type
                    self.scrape_player_page(path, name, pos)
    ...
    def scrape_player_page(self, path, team, name, pos):
        ...
        for row in body.findAll("tr"):
            ...
            self.writer.writerow([player_id] + [name] + [pos] + [dob] + [team] + stats) #WRITE: standard table row
...
scraper = fangraphs_scraper("al","pitcher", "2013", "standard")
scraper.scrape()
